...
export const useViewportIntersect = (elementRef: RefObject<HTMLDivElement | null>) => {
  ...
  useEffect(() => {
    ...
    const observerEntry = new IntersectionObserver((entries) => {
      ...
      rootMargin: '0px',
      threshold: 0.10 // Trigger when at least 42% of the element is visible
    });
    const observerFullVisible = new IntersectionObserver((entries) => {
      // entries[0] is single observed element instance
      const entry = entries[0];
      const isIntersecting = entry.isIntersecting
